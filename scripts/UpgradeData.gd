...
static func get_all_upgrades() -> Array:
	return [
		...
		{id = "rapid_fire",    name = "Rapid Fire",     description = "Fire rate +15%",                                    tier = "common"},
		{id = "piercing_shot", name = "Piercing Shot",  description = "Pierce 1 enemy, +5% pierce dmg",                    tier = "rare"},
		{id = "slow_touch",    name = "Slow Touch",     description = "15% slow for 1.5s",                                  tier = "rare"},
		{id = "leech_shot",    name = "Leech Shot",     description = "Kill: +3 HP",                                        tier = "rare"},
		{id = "sniper_shot",   name = "Sniper Shot",    description = "+20% dmg, +5% fire delay",                           tier = "rare"},
		...
		{id = "explosive_tip", name = "Explosive Tip",  description = "30% AOE dmg, r=48px",                               tier = "epic"},
		{id = "bouncing_shot", name = "Bouncing Shot",  description = "Bounce 1 time",                                      tier = "epic"},
		{id = "xp_shot",       name = "XP Shot",        description = "+1 XP per hit",                                      tier = "epic"},
	]
...
static func next_level_description(id: String, current_level: int) -> String:
	...
	match id:
		...
		"explosive_tip":
			...
			var s: String = str(ratio[mini(next, 10)]) + "% AOE  r=" + str(int(radius)) + "px"
			if next >= 8:
				s += "  2x blast"
			if next >= 9:
				s += "  ignites"
...
static func get_random_upgrades(count: int, upgrade_levels_dict: Dictionary = {}) -> Array:
	...
	for u in available:
		match u.tier:
			...
			"rare":   weights.append(3)
			"epic":   weights.append(1)
